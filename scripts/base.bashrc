...
DOCKERFILE_ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd -P)"
# echo "==============DOCKERFILE_ROOT_DIR:${DOCKERFILE_ROOT_DIR}"
PROJECT_IN_DOCKER=false

# If inside docker container
if [ -f /.dockerenv ]; then
  PROJECT_IN_DOCKER=true
  DOCKERFILE_ROOT_DIR="$DOCKERFILE_ROOT_DIR"
fi

export DOCKERFILE_ROOT_DIR="${DOCKERFILE_ROOT_DIR}"
export PROJECT_IN_DOCKER="${PROJECT_IN_DOCKER}"
export ZARK_CACHE_DIR="${DOCKERFILE_ROOT_DIR}/.cache"
export TAB="    " # 4 spaces

source ${DOCKERFILE_ROOT_DIR}/scripts/common.bashrc
...
function building() {
  (echo >&2 -e "[${GREEN}${BOLD} Building ${NO_COLOR}] $*")
}

function releasing() {
  (echo >&2 -e "[${GREEN}${BOLD} Releasing ${NO_COLOR}] $*")
}
...
function run() {
  if [ "${VERBOSE}" = yes ]; then
    ...
    local errfile="${DOCKERFILE_ROOT_DIR}/.errors.log"
    echo "${@}" >"${errfile}"
    if ! "${@}" >>"${errfile}" 2>&1; then
      local exitcode=$?
      cat "${errfile}" 1>&2
...
function git_sha1() {
  if [ -x "$(which git 2>/dev/null)" ] &&
    [ -d "${DOCKERFILE_ROOT_DIR}/.git" ]; then
